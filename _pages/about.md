...
I build LLM systems end to end, centered on two questions: how agents **collaborate with humans** and with each other, and how they **personalize** to the user in front of them. On the collaboration side, I design conversational agents that track what a user needs as a dialogue unfolds and multi-agent pipelines where models coordinate retrieval, generation, and verification with humans in the loop. On the personalization side, I adapt model behavior to individual users and use cases, from response style and reading level to safety behavior, using RAG, RL, PEFT/LoRA fine-tuning, and quantization-aware training. I target high-stakes applications where personalization and trust decide whether a system ships: content moderation, misinformation response, and crisis communication. My work spans three areas:
